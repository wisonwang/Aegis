...
curl -s -X POST "$BASE/api/v1/datasources/demo/query/estimate" \
  ...
  -d '{"sql":"SELECT * FROM customers"}' | python3 -m json.tool

echo; echo "== 4. Run a governed query (row policy + column masking applied) =="
curl -s -X POST "$BASE/api/v1/datasources/demo/query" \
  -H "$AUTH" -H 'Content-Type: application/json' \
  -d '{"sql":"SELECT id,name,phone FROM customers"}' | python3 -m json.tool
...
curl -s -X POST "$BASE/api/v1/datasources/demo/metrics/customer_count/run" \
