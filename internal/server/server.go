...
func Run(cfg *config.Config) error {
	...
	if cfg.SeedDemo {
		...
		if err := reconcileDemoDatasource(st, cfg); err != nil {
			logging.With("error", err.Error()).Warn("demo datasource reconcile failed")
		}
	}

	// Observability: advertise build identity and live counts at start-up.
	metrics.SetBuildInfo(version.Version, version.Commit)
	// Instance-wide gauges must count every tenant, not just "default".
	allWS := store.WithWorkspace(context.Background(), store.WorkspaceAll)
	if dss, err := st.ListDataSources(allWS); err == nil {
		metrics.SetDatasources(len(dss))
	}
	if dsets, err := st.ListDatasets(allWS); err == nil {
		n := 0
		for _, d := range dsets {
	...
	if cfg.IsProduction() {
		logging.With("component", "network").Warn("Aegis serves plain HTTP only; terminate TLS at a reverse proxy or load balancer before exposing it externally")
	}
...
func registerRoutes(engine *gin.Engine, h *api.Handler, st *store.Store, px *proxy.Proxy, cfg *config.Config, oidcH *api.OIDCHandler, ldapH *api.LDAPHandler, caps *capabilities.Capabilities) {
	...
	engine.GET("/api/v1/ready", gin.WrapF(func(w http.ResponseWriter, r *http.Request) {
		...
		if _, err := st.ListDataSources(store.WithWorkspace(r.Context(), store.WorkspaceAll)); err != nil {
	...
	// ---- Instance stats (ADR-0005, Phase 1): privacy-safe adoption snapshot ----
	engine.GET("/admin/api/stats", gin.WrapF(a(h.AdminStats)))
